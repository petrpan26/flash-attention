...
void run_mha_fwd_grouped(Flash_fwd_params &params, cudaStream_t stream) {
    FP16_SWITCH(!params.is_bf16, [&] {
        HEADDIM_SWITCH(params.d, [&] {
            BOOL_SWITCH(params.is_causal, Is_causal, [&] {
...
mha_varlen_fwd_grouped(
    ...
    // =============================================================================
    // PHASE 2: FULL GROUPED KERNEL INTEGRATION
    // =============================================================================
    // Use the real grouped CUDA kernel with K,V sharing across groups

    auto stream = at::cuda::getCurrentCUDAStream().stream();

    // Prepare host-side metadata
    std::vector<int> host_num_m_blocks(num_groups);
    std::vector<int> host_max_seqlen_k(num_groups);
    std::vector<void*> host_o_ptrs(num_groups);
    std::vector<void*> host_lse_ptrs(num_groups);

    constexpr int kBlockM = 128;  // For headdim 128 kernels
    for (int i = 0; i < num_groups; i++) {
        int total_q = q_list[i].size(0);
        host_num_m_blocks[i] = (total_q + kBlockM - 1) / kBlockM;
        host_max_seqlen_k[i] = max_seqlen_k_list[i];
        host_o_ptrs[i] = out_list[i].data_ptr();
        host_lse_ptrs[i] = softmax_lse_list[i].data_ptr();
    }
    ...
    cudaMemcpy(d_group_num_m_blocks, host_num_m_blocks.data(), num_groups * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_group_max_seqlen_k, host_max_seqlen_k.data(), num_groups * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_group_o_ptrs, host_o_ptrs.data(), num_groups * sizeof(void*), cudaMemcpyHostToDevice);
    cudaMemcpy(d_group_lse_ptrs, host_lse_ptrs.data(), num_groups * sizeof(void*), cudaMemcpyHostToDevice);

    // Set up params for grouped kernel
    Flash_fwd_params params;
    set_params_fprop(params,
                     cu_seqlens_q_list[0].size(0) - 1,  // batch size (assuming same across groups)
                     max_seqlen_q_list[0],              // Use first group's max seqlen_q
                     max_seqlen_k_list[0],              // Use first group's max seqlen_k (largest)
                     max_seqlen_q_list[0],              // seqlen_q_rounded
                     max_seqlen_k_list[0],              // seqlen_k_rounded
                     num_heads,
                     num_heads_k,
                     head_size,
                     head_size,                         // d_rounded
                     q_list[0],                         // Use first group's Q for base pointer
                     k,
                     v,
                     out_list[0],                       // Will be overridden by group_o_ptrs
                     cu_seqlens_q_list[0].data_ptr(),  // Will use first group's cu_seqlens_q
                     cu_seqlens_k_list[0].data_ptr(),  // Will use first group's cu_seqlens_k
                     nullptr,                           // seqused_k
                     nullptr,                           // p_ptr (softmax output)
                     softmax_lse_list[0].data_ptr(),   // Will be overridden by group_lse_ptrs
                     p_dropout,
                     softmax_scale,
                     window_size_left,
                     window_size_right,
    ...
    if (p_dropout > 0.0f) {
        auto gen = at::get_generator_or_default<at::CUDAGeneratorImpl>(
            gen_, at::cuda::detail::getDefaultCUDAGenerator());
        params.philox_args = gen->philox_cuda_state(0);
    }
    ...
    // For compatibility, return empty tensors for S_dmask and rng_state
    at::Tensor S_dmask = torch::empty({0}, opts);
