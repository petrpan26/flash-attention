...
void run_mha_fwd_grouped_<cutlass::bfloat16_t, 128, false>(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    if (params.num_groups == 2) {
        ...
        DROPOUT_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
            if (is_sm8x) {
                run_flash_fwd_2groups_smem_share<Flash_fwd_kernel_traits<Headdim, 128, 32, 4, false, false, cutlass::bfloat16_t>, Is_dropout, false>(
        ...
        DROPOUT_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
            if (is_sm8x) {
                run_flash_fwd_grouped_cache_aware<Flash_fwd_kernel_traits<Headdim, 128, 32, 4, false, false, cutlass::bfloat16_t>, Is_dropout, false>(
                    params, stream, grid_size_m);
            } else {
                run_flash_fwd_grouped_cache_aware<Flash_fwd_kernel_traits<Headdim, 128, 32, 4, false, false, cutlass::bfloat16_t>, Is_dropout, false>(
                    params, stream, grid_size_m);
    ...
    if (params.num_groups == 2) {
        ...
        DROPOUT_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
            if (is_sm8x) {
                ...
                run_flash_fwd_2groups_smem_share<Flash_fwd_kernel_traits<Headdim, 128, 32, 4, false, false, cutlass::bfloat16_t>, Is_dropout, true>(
                    params, stream, grid_size_m);
            }
        });
    } else {
